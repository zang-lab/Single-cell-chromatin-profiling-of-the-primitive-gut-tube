...
def get_Command(Ch,Start,End,bw,interval):
    cmd1 = ["bigWigSummary",bw,Ch,Start,End,interval]
    cmd1 = ' '.join([ str(i) for i in cmd1 ])
    return cmd1

def get_Signal(inputfile,outputfile,bw,interval):
    ...
    while line:
        ...
        S = int(array[1])
        E = int(array[2])
        cmd1 = get_Command(Ch,S,E,bw,interval)
...
#========GET OPTIONS========
    ...
    interval = 1
    outfile=outdir+name+".peak_1bin.bed"
    get_Signal(bed,outfile,bw,interval)
